  ...
  useEffect(() => {
    ...
    (async () => {
      const [p, v, e, t, n, m] = await Promise.all([
        ...
        supabase.from("events").select("id,name,event_date,start_time,status,client_name,deposit_required,deposit_paid,payment_status").eq("owner_id", user.id).order("event_date", { ascending: true, nullsFirst: false }).limit(50),
        supabase.from("tasks").select("id,title,due_date,status,priority").eq("assigned_to", user.id).neq("status", "done").order("due_date", { ascending: true, nullsFirst: false }).limit(10),
